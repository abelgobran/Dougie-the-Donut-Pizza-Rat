...
const displayResolve = document.getElementById("displayResolve")
     ...
     function buttonShow (resolution) {
        resolveDocDiv.innerHTML = `<div class= "resolution"><p id = "hidden">"${resolution}"</p></div>`
    ...
    fetch(brooklynComplaints)
        ...
        .then((brooklynInfo) => {
            for (let i = 0; i < brooklynInfo.length; i++) {
                ...
                resolveDiv.innerHTML += `<button id="displayResolve" onclick="buttonShow('${resolution}')"  >NYPD Resolve</button>` 
                // resolveDocDiv.innerText = 
